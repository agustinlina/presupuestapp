...
module.exports = async (req, res) => {
  ...
  // Leyenda: alineado a la izquierda, mayor espacio abajo, solo una línea
  doc.moveDown(2.3)
  doc
    .font('Helvetica-Bold')
    .fontSize(15)
    .fillColor('#000')
    .text('Presupuesto por Ud. requerido', 36, 148, {
      align: 'left',
      width: 500,
      continued: false
    })
  // Espacio extra debajo de la leyenda
  let leyendaY = doc.y + 25

  // Tabla de productos
  doc.font('Helvetica-Bold').fontSize(11).fillColor('#000')
  const col = [36, 110, 340, 410, 510]
  const tableTop = leyendaY
  doc.text('Cant.', col[0], tableTop, {
    width: col[1] - col[0],
    align: 'center'
  })
  doc.text('Descripción', col[1], tableTop, {
    width: col[2] - col[1],
    align: 'center'
  })
  doc.text('Precio/U', col[2], tableTop, {
    width: col[3] - col[2],
    align: 'center'
  })
  doc.text('Total', col[3], tableTop, {
    width: col[4] - col[3],
    align: 'center'
  })
  ...
  productos.forEach(prod => {
    ...
    doc.text(`$${precio.toFixed(2)}`, col[2], y, {
      width: col[3] - col[2],
      align: 'center'
    })
    doc.text(`$${total.toFixed(2)}`, col[3], y, {
      width: col[4] - col[3],
      align: 'center'
    })
    y += 22
  ...
  doc.text('Total:', col[2], y + 10, {
    width: col[3] - col[2],
    align: 'center'
  })
  doc.text(`$${totalGeneral.toFixed(2)}`, col[3], y + 10, {
    width: col[4] - col[3],
    align: 'center'
  })
